...
struct StochasticGrossPitaevskiiProblem{form,Tnoise,N,T1,T2,T3,T4,T5,T6,T7}
    ...
    function StochasticGrossPitaevskiiProblem(dispersion!, potential!, nonlinearity, pump!, u₀, lengths,
        ::Type{form}, ::Type{Tnoise}, param=()) where {form,Tnoise}

        base_prob = GrossPitaevskiiProblem(dispersion!, potential!, nonlinearity, pump!, u₀, lengths, param)
